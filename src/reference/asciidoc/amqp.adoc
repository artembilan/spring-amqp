...
NOTE: The listener container `beanName` is used as a `threadNamePrefix` for default `SimpleAsyncTaskExecutor`.
It is useful for logs analysis.
The provided `TaskExecutor` is used as is, without modification.
The custom `threadNamePrefix` for better logging should be supplied on that `TaskExecutor` bean definition.
